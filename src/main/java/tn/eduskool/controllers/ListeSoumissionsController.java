...
public class ListeSoumissionsController implements BaseController {
    ...
    public void chargerSoumissions() {
        if (utilisateur != null) {
            List<SoumissionDevoir> soumissions;
            if ("etudiant".equals(utilisateur.getType_Utilisateur())) {
                soumissions = soumissionService.recupererParEtudiant(utilisateur.getIdUtilisateur());
            } else {
                soumissions = soumissionService.recuperer();
            }
